...
<div class="popup open-popup" id="popup">
        ...
        <button type="button" class="popupbutton" onclick="document.getElementById('popup').classList.remove('open-popup');" style="margin-left: 1rem">
            Thanks
        </button>
</div>
@enderror
    ...
    <div class="main-container">
        ...
        <form action="/users/authenticate" method="post">
            ...
            <div class="email form__group field">
                <input type="email" class="form__field"  placeholder="Email" name="email" id="email" value="{{old('email')}}" required />
                <label for="name" class="form__label">Email</label>
            </div>

            <div class="pass form__group field">
                <input type="password" class="form__field"  placeholder="Password" name="password" id="password"value="{{old('password')}}"
                    required />                           
                        @error('password')
                        <p style="position: absolute; color:#ea4646">{{$message}}</p>
                        @enderror
                <label for="name" class="form__label">Password</label>
                <img id="passpic" class="img-pass" value="show" src="{{ asset('assets/img/show.png') }}" onclick="PassChange()" />
                <label class="switch">
                    <input type="checkbox" />
